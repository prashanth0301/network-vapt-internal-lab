...
async def _severity_distribution(session: AsyncSession) -> List[SeveritySlice]:
...
async def _vulnerability_trend(session: AsyncSession) -> List[TrendPoint]:
    day_expr = func.date(Vulnerability.created_at)
    result = await session.execute(
        select(day_expr.label("day"), func.count(Vulnerability.id))
        .where(Vulnerability.created_at.is_not(None))
        .group_by(day_expr)
    )
...
async def _top_open_ports(session: AsyncSession) -> List[PortSlice]:
    result = await session.execute(
...
async def _service_distribution(session: AsyncSession) -> List[ServiceSlice]:
    name_expr = func.coalesce(Service.normalized_name, Service.name)
    result = await session.execute(
        select(name_expr.label("name"), func.count(Service.id))
...
async def _recent_reports(session: AsyncSession) -> List[ReportItem]:
    result = await session.execute(
...
async def _top_vulnerable_hosts(session: AsyncSession) -> List[HostVulnItem]:
    result = await session.execute(
        select(Host.ip_address, func.max(Host.hostname), func.count(Vulnerability.id))
        .join(Vulnerability, Vulnerability.host_id == Host.id)
        .group_by(Host.ip_address)
...
async def _totals(session: AsyncSession) -> DashboardTotals:
...
async def _scan_duration_stats(session: AsyncSession) -> ScanDurationStats:
...
async def get_dashboard_summary(session: AsyncSession) -> DashboardSummary:
    distribution = await _severity_distribution(session)
    ...
    exploit_count = await session.scalar(
        select(func.count(func.distinct(CVE.id))).where(
    ...
    return DashboardSummary(
        ...
        vulnerability_trend=await _vulnerability_trend(session),
        top_open_ports=await _top_open_ports(session),
        service_distribution=await _service_distribution(session),
        recent_assessments=await _recent_assessments(session),
        recent_reports=await _recent_reports(session),
        top_vulnerable_hosts=await _top_vulnerable_hosts(session),
        ...
        scan_duration_stats=await _scan_duration_stats(session),
